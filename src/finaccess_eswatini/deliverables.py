"""Machine-checkable deliverable contracts for completed project phases."""
...
PHASE_DELIVERABLES = {
    ...
    12: (
        "notebooks/12_deployment.ipynb",
        "src/finaccess_eswatini/phase12_deployment.py",
        "scripts/run_phase12_deployment.ps1",
        "scripts/build_phase12_notebook.py",
        "tests/test_phase12_deployment.py",
        "frontend/vercel.json",
        "frontend/.vercelignore",
        "frontend/backend/main.py",
        "frontend/backend/service.py",
        "frontend/backend/schemas.py",
        "frontend/backend/requirements.txt",
        ".env.example",
        "reports/phase_12/deployment_validation.json",
        "reports/phase_12/phase12_summary.json",
        "reports/phase_12/deployment_report.md",
        "reports/phase_12/deliverable_checklist.md",
        "reports/phase_12/regression_validation.json",
    ),
    13: (
        "notebooks/13_portfolio_polish.ipynb",
        "README.md",
        "notebooks/README.md",
        "src/finaccess_eswatini/phase13_portfolio.py",
        "scripts/build_phase13_notebook.py",
        "scripts/capture_phase13_screenshots.mjs",
        "scripts/run_phase13_portfolio.ps1",
        "scripts/run_phase13_regression.ps1",
        "scripts/write_phase13_regression.py",
        "tests/test_phase13_portfolio.py",
        "reports/phase_13/screenshots/01_overview.png",
        "reports/phase_13/screenshots/02_assessment.png",
        "reports/phase_13/screenshots/03_assessment_results.png",
        "reports/phase_13/screenshots/04_methodology.png",
        ...
        "reports/project_documentation/FinAccess_Eswatini_Phases_1_to_13_Report.docx",
    ),
}


def validate_completed_phase_files(
    project_root: Path, through_phase: int | None = None
) -> dict[int, list[str]]:
    """Return missing required files by phase; an empty mapping is a pass."""

    phases = {
        phase: paths
        for phase, paths in PHASE_DELIVERABLES.items()
        if through_phase is None or phase <= through_phase
    }
    missing = {
        phase: [relative for relative in paths if not (project_root / relative).is_file()]
        for phase, paths in phases.items()
    }
    return {phase: paths for phase, paths in missing.items() if paths}


def render_delivery_checklist(project_root: Path, through_phase: int | None = None) -> str:
    """Render a transparent checklist covering every completed phase."""

    missing = validate_completed_phase_files(project_root, through_phase)
    lines = [
    ...
    for phase, paths in PHASE_DELIVERABLES.items():
        ...
        missing_for_phase = set(missing.get(phase, []))
        for relative in paths:
            marker = "❌" if relative in missing_for_phase else "✅"
    ...
    lines.extend(
        [
            "## Result",
            "",
            f"**PASS** — all required files through Phase {through_phase or max(PHASE_DELIVERABLES)} are present."
            if not missing
            else "**FAIL** — one or more required completed-phase files are missing.",
            "",
        ]
    )
    return "\n".join(lines)
