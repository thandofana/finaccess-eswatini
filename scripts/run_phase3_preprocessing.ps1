...
if (-not $ProcessedDirectory) {
    $ProcessedDirectory = Join-Path $ProjectRoot "data\processed"
}
if (-not $ReportDirectory) {
    $ReportDirectory = Join-Path $ProjectRoot "reports\phase_3"
}

$env:PYTHONPATH = Join-Path $ProjectRoot "src"
$env:OPENBLAS_NUM_THREADS = "1"

& $EnvironmentPython -m finaccess_eswatini.phase3_preprocessing `
    --raw $RawData `
    --dictionary $Dictionary `
    --processed-dir $ProcessedDirectory `
    --output-dir $ReportDirectory
if ($LASTEXITCODE -ne 0) {
    throw "Phase 3 preprocessing failed."
}
