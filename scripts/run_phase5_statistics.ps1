...
if (-not $ReportDirectory) {
    $ReportDirectory = Join-Path $ProjectRoot "reports\phase_5"
}

$env:PYTHONPATH = Join-Path $ProjectRoot "src"
$env:OPENBLAS_NUM_THREADS = "1"

& $EnvironmentPython -m finaccess_eswatini.phase5_statistics `
    --raw $RawData `
    --dictionary $Dictionary `
    --output-dir $ReportDirectory
if ($LASTEXITCODE -ne 0) {
    throw "Phase 5 statistical analysis failed."
}
